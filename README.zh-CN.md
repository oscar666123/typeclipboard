...
> [!IMPORTANT]
> 启动 Type Clipboard 前必须完整解压 ZIP 压缩包。直接在压缩包预览窗口中运行 `TypeClipboard.exe`，可能使用临时解压目录，造成 **Auto loaded** 提示反复出现、窗口闪烁，以及缓存或设置冲突。
...
1. 在 GitHub Release 下载最新版 `TypeClipboard-Portable-vX.Y.Z.zip`。
2. 右键 ZIP 并选择 **全部解压缩**，也可以使用常用压缩工具完整解压。
3. 打开解压后的文件夹，从该文件夹运行 `TypeClipboard.exe`。

每次都从完整解压后的文件夹启动。压缩包预览工具可能从临时目录运行 EXE，导致程序无法稳定访问缓存和设置。

## 使用流程

1. 确认 `TypeClipboard.exe` 从完整解压后的文件夹运行。
2. 在本机正常复制文本。
3. 打开 **Type Clipboard**，预览框会自动更新。
4. 把光标放进目标 RDP、服务器或应用的输入框，再点击 **Type**；程序会自动恢复该目标窗口。也可以在目标窗口中直接按全局 Type 快捷键，默认是 F9。
5. 程序恢复目标窗口后等待设定的开始延迟，然后从当前获得焦点的输入位置开始输入。
6. 需要中断时按 Emergency 快捷键、Stop 快捷键，或点击 **Stop**；默认按键是 F8 和 F10。
7. 点击 **Type History** 或按 Ctrl+H，可重新使用以前输入过的文本。
...
## Auto loaded 反复提示或窗口闪烁

直接从 ZIP 压缩包内部启动 EXE 时，可能出现 **Auto loaded** 反复提示、状态快速刷新或窗口持续闪烁。

1. 关闭全部 Type Clipboard 实例。
2. 把完整 ZIP 解压到固定文件夹，例如 `C:\Tools\TypeClipboard`。
3. 从解压后的文件夹启动 `TypeClipboard.exe`。
4. 后续版本也分别放入完整解压后的文件夹运行。
...
- 压缩包内部或压缩包预览窗口不属于支持的运行位置；临时解压机制可能引发缓存、设置和剪贴板状态重复更新冲突。
