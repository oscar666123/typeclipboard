...
- Type 和 Stop 的窗口内快捷键可分别选择，并按 Windows 用户保存。
- 支持窗口置顶，默认启用。
...
3. 点击 **Type** 或按所选 Type 快捷键。
4. 在开始延迟结束前，把焦点切到目标 RDP、服务器或应用窗口。
5. 需要中断时按所选全局急停热键；软件窗口有焦点时也可以按所选 Stop 快捷键，或点击 **Stop**。
...
- **Type shortcut**：可选 Ctrl+T、Ctrl+Shift+T、Ctrl+Alt+T、F9 或 Disabled。
- **Stop shortcut**：可选 Esc、Ctrl+Shift+S、Ctrl+Alt+S、F10 或 Disabled。
- **Always on top**：让 Type Clipboard 保持在其他窗口上方，同时保留正常最小化功能。

快捷键和窗口置顶选择保存在 `%LOCALAPPDATA%\TypeClipboard\settings.json`。
...
脚本默认从项目文件读取版本号。需要构建指定数字版本时，可添加 `-Version 0.2.4`；ZIP 文件名和 EXE 版本信息会使用同一个值。
